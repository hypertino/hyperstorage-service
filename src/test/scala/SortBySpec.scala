...
class SortBySpec extends FreeSpec with Matchers {
  "Sort" - {
    "should decode" in {
      Sort.parseQueryParam("field1") should equal(Seq(SortBy("field1")))
      Sort.parseQueryParam("-field1") should equal(Seq(SortBy("field1", descending = true)))
      Sort.parseQueryParam("field1,field2") should equal(Seq(SortBy("field1"),SortBy("field2")))
      Sort.parseQueryParam("-field1,+field2") should equal(Seq(SortBy("field1", descending = true),SortBy("field2")))
      Sort.parseQueryParam("") should equal(Seq.empty)
      Sort.parseQueryParam("   ") should equal(Seq.empty)
      Sort.parseQueryParam(" -field1  , +field2") should equal(Seq(SortBy("field1", descending = true),SortBy("field2")))
    }

    "should decode implicit from HRL" in {
